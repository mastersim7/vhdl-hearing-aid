...
% Set filter number first!!
filter = 1;
switch filter
    case 1
        cells = 'A2:A41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_1.tv';
    case 2
        cells = 'B2:B41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_2.tv';
    case 3
        cells = 'C2:C41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_3.tv';
    case 4
        cells = 'D2:D41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_4.tv';
    case 5
        cells = 'E2:E41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_5.tv';
    case 6
        cells = 'F2:F41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_6.tv';
    case 7
        cells = 'G2:G41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_7.tv';
    case 8
        cells = 'H2:H41'; %for reading coefficients from file
        tvfile = 'pre_output_mac_8.tv';
    otherwise msgbox('We have only 8 filters, you know...', 'Error :-(');
end;
...
% Output values
pre_output_n = 37;  % result of adding all the results of multiplications: multi_result
...
% Read coefficients instead of generating from file samples.xls which is produced by 'samples_generator.m'
[num, samples_bin_cell, raw] = xlsread('samples.xls', 'Sheet1', 'A1:A80');
for i = 1 : samples_num
    samples_int(i) = bin2int(char(samples_bin_cell(i)));
...
[num, coeff_bin_cell, raw] = xlsread('Coeffs.xls', 'Sheet1', cells);
for i = 1 : coeff_num
...
pre_output_mac = zeros(length(coeff_x_summed_samples_int), 1);
pre_output_mac_bin = '';
for i = 1:length(coeff_x_summed_samples_int)
	if i == 1 
		pre_output_mac(i) = coeff_x_summed_samples_int(1);
	else
		pre_output_mac(i) = pre_output_mac(i-1) + coeff_x_summed_samples_int(i);
	end;
	pre_output_mac_bin(i,:) = int2bin(pre_output_mac(i), pre_output_n);
end;
...
% file for mac values:
fid_pre_output_mac = fopen(tvfile, 'w');
...
% Writing mac values
fprintf(fid_pre_output_mac, '%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n', pre_output_mac_bin');
